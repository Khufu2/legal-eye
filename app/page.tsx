...
function LegalEye() {
  ...
  return <div className={"legal-app view-"+view}>
    ...
    <div className="workspace-stage">
      ...
      <main key={identity?.user.id||"signed-out"} className="workspace-main">{content}</main>
    ...
    <Dialog open={authOpen} onOpenChange={setAuthOpen}><DialogContent><DialogHeader><DialogTitle>{identity?"Organization access":"Sign in to LOCKE"}</DialogTitle><DialogDescription>{identity?"Your secure organization session is active for this browser tab.":"Use the credentials issued by your LOCKE organization owner."}</DialogDescription></DialogHeader>{identity?<div className="auth-session"><ShieldCheck/><div><b>{identity.user.email}</b><span>{identity.role} · private organization workspace</span></div><Button variant="outline" onClick={signOut}>Sign out</Button></div>:<div className="auth-form"><label>Email<Input type="email" value={email} onChange={event=>setEmail(event.target.value)} autoComplete="username"/></label><label>Password<Input type="password" value={password} onChange={event=>setPassword(event.target.value)} onKeyDown={event=>event.key==="Enter"&&signIn()} autoComplete="current-password"/></label><Button onClick={signIn} disabled={signingIn||!email||!password}>{signingIn?"Signing in…":"Sign in"}</Button></div>}</DialogContent></Dialog>
