...
function LegalEye() {
  ...
  return <SidebarProvider defaultOpen style={{"--sidebar-width":"12.75rem","--sidebar-width-icon":"3.5rem"} as React.CSSProperties}>
    ...
    <Dialog open={authOpen} onOpenChange={setAuthOpen}><DialogContent><DialogHeader><DialogTitle>{identity?"Demo organization":"Connect test organization"}</DialogTitle><DialogDescription>{identity?"The Supabase owner session is active for this browser tab.":"Use the temporary owner credentials supplied by Legal Eye."}</DialogDescription></DialogHeader>{identity?<div className="auth-session"><ShieldCheck/><div><b>{identity.user.email}</b><span>{identity.role} · Legal Eye Client Demonstration</span></div><Button variant="outline" onClick={signOut}>Sign out</Button></div>:<div className="auth-form"><label>Email<Input type="email" value={email} onChange={event=>setEmail(event.target.value)} autoComplete="username"/></label><label>Password<Input type="password" value={password} onChange={event=>setPassword(event.target.value)} onKeyDown={event=>event.key==="Enter"&&signIn()} autoComplete="current-password"/></label><Button onClick={signIn} disabled={signingIn||!email||!password}>{signingIn?"Connecting…":"Connect organization"}</Button></div>}</DialogContent></Dialog>
