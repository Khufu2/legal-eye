...
import {
  Activity, Archive, Menu, Moon, Sun, ArrowUpRight, BookOpen, Bot, BriefcaseBusiness, Check, ChevronDown,
  ...
  Database, Eye, KeyRound, Network, ShieldCheck, Sparkles, Table2, Trash2, Upload, Users, Workflow, X,
} from "lucide-react";
...
import { ResearchHome, ResearchLive, LockeMark } from "@/components/legal-eye-research";
...
import { LockeAssistant } from "@/components/locke-assistant";
...
function VaultView({identity,documents,connect,refresh}:{identity:WorkspaceIdentity|null;documents:VaultDocument[];connect:()=>void;refresh:()=>void}) {
  ...
  const input=useRef<HTMLInputElement>(null),[uploading,setUploading]=useState(false),[filter,setFilter]=useState(""),[showArchived,setShowArchived]=useState(false),[preview,setPreview]=useState<{title:string;text:string}|null>(null);
  ...
  const archiveDocument=async(id:string,title:string)=>{if(!identity)return;if(!window.confirm(`Archive "${title}"? It will stop appearing in document pickers, but the record remains auditable.`))return;try{await workspace(identity,`documents?id=eq.${id}&organization_id=eq.${identity.organization_id}`,{method:"PATCH",body:JSON.stringify({status:"archived",updated_at:new Date().toISOString()})});toast.success("Document archived");refresh();}catch(e){toast.error(e instanceof Error?e.message:"Document could not be archived");}};
  ...
  const upload=async(file?:File)=>{
    ...
    try{
      ...
      const created=await fetch(SUPABASE_URL+"/rest/v1/documents",{method:"POST",headers:{apikey:SUPABASE_KEY,Authorization:"Bearer "+identity.access_token,"content-type":"application/json",Prefer:"return=minimal"},body:JSON.stringify({id:documentId,organization_id:identity.organization_id,title:file.name.replace(/\.[^.]+$/,""),file_name:file.name,mime_type:file.type||null,storage_path:storagePath,size_bytes:file.size,status:"uploaded",jurisdiction_codes:["TZ"],uploaded_by:identity.user.id,metadata:{upload_channel:"legal-eye-web",classification:"confidential"}})});
      if(!created.ok)throw new Error((await created.json().catch(()=>null))?.message||"Document record could not be created");
      const process=await fetch("/api/process-document",{method:"POST",headers:{Authorization:"Bearer "+identity.access_token,"content-type":"application/json"},body:JSON.stringify({document_id:documentId,organization_id:identity.organization_id})});
    ...
    }catch(error){setUploadError(error instanceof Error?error.message:"Upload failed");refresh()}finally{setUploading(false);if(input.current)input.current.value=""}
  };
  const display=documents.filter(d=>(showArchived||d.status!=="archived")&&`${d.title} ${d.file_name}`.toLowerCase().includes(filter.toLowerCase())).map(d=>[d.id,d.title,d.status,d.file_name||"Private document"]);
  return <div className="view-pad"><Header title="Private knowledge" meta="Upload client and firm documents here, then use Ready files across LOCKE." action={<><input ref={input} hidden type="file" accept=".pdf,.docx,.xlsx,.txt" onChange={event=>upload(event.target.files?.[0])}/><Button onClick={()=>identity?input.current?.click():connect()} disabled={uploading}><Upload/> {uploading?"Securing…":"Upload"}</Button></>}/><div className="workspace-tip"><FolderLock/><span><b>Document flow:</b> Upload a file → wait for <strong>Ready</strong> → select it in Review, Tables, Skills, Agent or Workflows. Archive old test files here before handing the workspace to a lawyer.</span></div>{uploadError && <div className="inline-error" role="alert">{uploadError}</div>}<div className="vault-toolbar"><Input aria-label="Search private documents" placeholder="Search your documents…" value={filter} onChange={e=>setFilter(e.target.value)}/><label><input type="checkbox" checked={showArchived} onChange={e=>setShowArchived(e.target.checked)}/> Show archived</label></div><div className="vault-grid"><section>{display.length?display.map(x=><div className={"vault-document status-"+x[2]} key={x[0]}><i><FolderLock/></i><span><b>{x[1]}</b><small><strong>{x[2]}</strong> · {x[3]}</small></span>{x[2]==="ready"&&<><Button variant="ghost" onClick={()=>void viewText(x[0])}>Open text</Button><Button variant="outline" onClick={()=>window.location.assign(`/?view=agent&document_id=${encodeURIComponent(x[0])}`)}><Bot/> Use in Agent</Button></>}{x[2]!=="ready"&&x[2]!=="archived"&&<Button variant="outline" onClick={()=>void retry(x[0])}>Retry processing</Button>}{x[2]!=="archived"&&<Button variant="ghost" className="destructive-link" onClick={()=>void archiveDocument(x[0],x[1])}><Archive/> Archive</Button>}{["owner","admin","partner"].includes(identity?.role||"")&&<Button variant="ghost" className="destructive-link" onClick={()=>void deleteDocument(x[0],x[1])}><Trash2/> Delete</Button>}</div>):<div className="empty-live"><FolderLock/><h2>{identity?"Your vault is empty":"Sign in to open the vault"}</h2><p>{identity?"Upload the first real client document.":"Private knowledge is available only to organization members."}</p></div>}</section><aside><ShieldCheck/><h2>Private Firm Intelligence</h2><p>Client content is permission-filtered before retrieval. Public and firm graphs stay separate and every agent action is auditable.</p>{[["Encryption","Supabase storage · private bucket"],["Data boundary","Organization + matter RLS"],["Model training","Disabled for private data"],["Parser","Docling + lightweight fallback"]].map(x=><div key={x[0]}><span>{x[0]}</span><b>{x[1]}</b></div>)}</aside></div><Dialog open={!!preview} onOpenChange={open=>{if(!open)setPreview(null)}}><DialogContent className="vault-preview"><DialogHeader><DialogTitle>{preview?.title}</DialogTitle><DialogDescription>Exact processed source text</DialogDescription></DialogHeader><pre>{preview?.text}</pre></DialogContent></Dialog></div>;
}
function ReviewView({identity,connect,documents,refresh}:{identity:WorkspaceIdentity|null;connect:()=>void;documents:VaultDocument[];refresh:()=>void}) {
  ...
  const [reviewInstructions,setReviewInstructions]=useState("Identify text-supported risks in termination, payment, confidentiality, liability and unilateral changes. Explain commercial imbalance without claiming illegality. Do not assume regulatory approvals or missing legal requirements without supplied authority.");
  ...
  const createPastedSource=async()=>{
    ...
    const created=await fetch(SUPABASE_URL+"/rest/v1/documents",{method:"POST",headers:{...authHeaders!,"content-type":"application/json",Prefer:"return=minimal"},body:JSON.stringify({id:documentId,organization_id:identity.organization_id,title:`Pasted contract · ${new Date().toLocaleDateString()}`,file_name:fileName,mime_type:"text/plain",storage_path:storagePath,size_bytes:blob.size,status:"uploaded",jurisdiction_codes:["TZ"],uploaded_by:identity.user.id,metadata:{upload_channel:"review-paste",classification:"confidential"}})});
    if(!created.ok)throw new Error((await created.json().catch(()=>null))?.message||"Pasted source record could not be created");
    await fetch("/api/process-document",{method:"POST",headers:{Authorization:"Bearer "+identity.access_token,"content-type":"application/json"},body:JSON.stringify({document_id:documentId,organization_id:identity.organization_id})});refresh();
    return documentId;
  };
  ...
  const runReview=async()=>{
    ...
    try{
      ...
      const response=await fetch("/api/legal-ai",{method:"POST",headers:{Authorization:"Bearer "+identity.access_token,"content-type":"application/json"},body:JSON.stringify({action:"review",organization_id:identity.organization_id,text:contract,playbook:reviewInstructions.split("\n").map(line=>line.trim()).filter(Boolean)})});
  ...
  return <div className="view-pad"><Header title="Review" meta="Review an agreement against its text and your instructions." action={<div className="row"><Button variant="outline" onClick={exportWord} disabled={!findings.length}>Export Word</Button><Button variant="outline" onClick={exportReview} disabled={!reviewProjectId}>JSON</Button><Button onClick={runReview} disabled={reviewing||contract.trim().length<20}><Sparkles/> {reviewing?"Reviewing…":"Run AI review"}</Button></div>}/>
    ...
    <label><span>Review instructions</span><Textarea aria-label="Review instructions" disabled={reviewing} value={reviewInstructions} maxLength={2000} onChange={e=>setReviewInstructions(e.target.value)} /></label>
    <Textarea disabled={reviewing} value={contract} onChange={event=>{setContract(event.target.value);if(selectedDocument){setSelectedDocument("");setLoadedTitle("Pasted agreement text")}resetResults()}} placeholder="Paste the agreement text, or select a processed vault document above..." aria-label="Contract text for review"/>
...
function LegalEye() {
  ...
  const [authOpen,setAuthOpen]=useState(false), [authMode,setAuthMode]=useState<"signin"|"signup"|"verify"|"workspace">("signin"),[email,setEmail]=useState(""), [password,setPassword]=useState(""),[firmName,setFirmName]=useState(""),[pendingSession,setPendingSession]=useState<WorkspaceIdentity|null>(null),[authError,setAuthError]=useState(""),[authBusy,setAuthBusy]=useState(false), [identity,setIdentity]=useState<WorkspaceIdentity|null>(null);
  const [pendingInvite,setPendingInvite]=useState("");
  const [availableFirms,setAvailableFirms]=useState<Array<{organization_id:string;role:string;organizations:{name:string}|null}>>([]);
  useEffect(()=>{if(!authOpen||!identity){setAvailableFirms([]);return;}let cancelled=false;void fetch(SUPABASE_URL+`/rest/v1/organization_members?select=role,organization_id,organizations(name)&user_id=eq.${identity.user.id}&is_active=is.true`,{headers:{apikey:SUPABASE_KEY,Authorization:"Bearer "+identity.access_token}}).then(async r=>{if(!r.ok)throw new Error("Could not load firm workspaces");const firms=await r.json();if(!cancelled)setAvailableFirms(firms);}).catch(e=>{if(!cancelled)setAuthError(e.message)});return()=>{cancelled=true}},[authOpen,identity]);
  ...
  const connectSession=async(data:{access_token:string;refresh_token?:string;expires_at?:number;user:{id:string;email?:string}},membership:{organization_id:string;role:string})=>{
    const next={access_token:data.access_token,refresh_token:data.refresh_token,expires_at:data.expires_at,user:data.user,role:membership.role,organization_id:membership.organization_id};
    sessionStorage.setItem("legal-eye-session",JSON.stringify(next));sessionStorage.removeItem("locke-pending-firm");setPendingSession(null);setMatters([]);setMatterId(null);setVaultDocuments([]);setIdentity(next);setPassword("");setAuthOpen(false);await refreshVault(next);if(researchQuery.trim()){setResearchRun(x=>x+1);setView("research")}toast.success("Workspace connected");
  };
  const acceptInvite=async(data:{access_token:string;refresh_token?:string;expires_at?:number;user:{id:string;email?:string}})=>{
    const response=await fetch(SUPABASE_URL+"/rest/v1/rpc/accept_workspace_invite",{method:"POST",headers:{apikey:SUPABASE_KEY,Authorization:"Bearer "+data.access_token,"content-type":"application/json"},body:JSON.stringify({p_token:pendingInvite})});
    const org=await response.json();if(!response.ok)throw new Error(org?.message||"Invitation could not be accepted");
    const membership=await fetch(SUPABASE_URL+`/rest/v1/organization_members?select=role,organization_id&organization_id=eq.${org}&user_id=eq.${data.user.id}&is_active=is.true`,{headers:{apikey:SUPABASE_KEY,Authorization:"Bearer "+data.access_token}});
    const memberships=membership.ok?await membership.json():[];
    if(!memberships[0])throw new Error("Invitation accepted, but membership could not be loaded. Retry to reconnect.");
    sessionStorage.removeItem("locke-pending-invite");setPendingInvite("");setMatterId(null);setMatters([]);
    await connectSession(data,memberships[0]);
  };
  const signUp=async()=>{
    if(authBusy)return;
    if(!email.trim()||(!pendingInvite&&!firmName.trim())||password.length<12){setAuthError("Enter your email, a password of at least 12 characters, and a firm name if you are creating a workspace.");return;}
    setAuthBusy(true);setAuthError("");
    try{
      const response=await fetch(SUPABASE_URL+`/auth/v1/signup?redirect_to=${encodeURIComponent(window.location.origin)}`,{method:"POST",headers:{apikey:SUPABASE_KEY,"content-type":"application/json"},body:JSON.stringify({email:email.trim(),password})});
      const data=await response.json();if(!response.ok)throw new Error(data?.msg||data?.error_description||data?.message||"Registration failed");
      sessionStorage.setItem("locke-pending-firm",firmName.trim());setPassword("");setAuthMode("verify");
    }catch(error){setAuthError(error instanceof Error?error.message:"Registration failed")}finally{setAuthBusy(false)}
  };
  const signIn=async()=>{
    if(authBusy)return;
    setAuthBusy(true);setAuthError("");
    try{
      ...
      const verified=await fetch(SUPABASE_URL+"/auth/v1/user",{headers:{apikey:SUPABASE_KEY,Authorization:"Bearer "+data.access_token}});
      const user=verified.ok?await verified.json():null;
      if(!user?.email_confirmed_at||user?.is_anonymous)throw new Error("Confirm your email address before creating a firm workspace.");
      if(pendingInvite){await acceptInvite(data);return;}
      ...
      if(!membership.ok)throw new Error("Your organization membership could not be checked. Please retry.");
      const memberships=await membership.json();
      if(!memberships[0]?.organization_id){setPendingSession(data);setFirmName(sessionStorage.getItem("locke-pending-firm")||"");setPassword("");pendingFirmSlug.current="";setAuthMode("workspace");return;}
      await connectSession(data,memberships[0]);
    }catch(error){setAuthError(error instanceof Error?error.message:"Sign in failed")}finally{setAuthBusy(false)}
  };
  const pendingFirmSlug=useRef("");
  const createFirm=async()=>{
    if(authBusy)return;
    if(!pendingSession||!firmName.trim()){setAuthError("Enter a firm or practice name.");return;}
    setAuthBusy(true);setAuthError("");
    try{
      const slug=pendingFirmSlug.current||(firmName.trim().toLowerCase().normalize("NFKD").replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,"").slice(0,42)||"practice")+"-"+crypto.randomUUID().slice(0,8);
      pendingFirmSlug.current=slug;
      const response=await fetch(SUPABASE_URL+"/rest/v1/rpc/create_organization_with_owner",{method:"POST",headers:{apikey:SUPABASE_KEY,Authorization:"Bearer "+pendingSession.access_token,"content-type":"application/json"},body:JSON.stringify({p_name:firmName.trim(),p_slug:slug})});
      const organizationId=await response.json();if(!response.ok)throw new Error(organizationId?.message||"Firm workspace could not be created");
      const check=await fetch(SUPABASE_URL+`/rest/v1/organization_members?select=role,organization_id&user_id=eq.${pendingSession.user.id}&organization_id=eq.${organizationId}&is_active=is.true`,{headers:{apikey:SUPABASE_KEY,Authorization:"Bearer "+pendingSession.access_token}});
      const memberships=check.ok?await check.json():[];
      if(!memberships[0])throw new Error("Workspace created, but access could not be verified. Retry to reconnect safely.");
      pendingFirmSlug.current="";
      await connectSession(pendingSession,memberships[0]);
    }catch(error){setAuthError(error instanceof Error?error.message:"Firm workspace could not be created")}finally{setAuthBusy(false)}
  };
  const signOut=()=>{const token=identity?.access_token;sessionStorage.removeItem("legal-eye-session");setIdentity(null);setPendingSession(null);setVaultDocuments([]);setMatters([]);setMatterId(null);setPassword("");setAuthMode("signin");setAuthOpen(false);if(token)void fetch(SUPABASE_URL+"/auth/v1/logout?scope=local",{method:"POST",headers:{apikey:SUPABASE_KEY,Authorization:"Bearer "+token}}).catch(()=>undefined);toast("Signed out")};
  useEffect(()=>{const fragment=new URLSearchParams(window.location.hash.replace(/^#/,""));const invite=fragment.get("invite")||sessionStorage.getItem("locke-pending-invite");if(invite&&/^[0-9a-f-]{36}$/i.test(invite)){setPendingInvite(invite);sessionStorage.setItem("locke-pending-invite",invite);setAuthOpen(true);if(fragment.has("invite"))window.history.replaceState(null,"",window.location.pathname+window.location.search);}const callback=readAuthCallback(window.location.hash);if(callback?.kind==="recovery"){window.location.replace("/reset-password"+window.location.hash);return;}if(callback){window.history.replaceState(null,"",window.location.pathname+window.location.search);setAuthOpen(true);if(callback.kind==="error")setAuthError(callback.message);else toast.success("Email confirmed. Sign in to continue.");}if(new URLSearchParams(window.location.search).get("signin")==="1")setAuthOpen(true)},[]);
  ...
  useEffect(()=>{if(!identity?.refresh_token)return;const session=identity;let cancelled=false;const renew=async()=>{try{const r=await fetch(SUPABASE_URL+"/auth/v1/token?grant_type=refresh_token",{method:"POST",headers:{apikey:SUPABASE_KEY,"content-type":"application/json"},body:JSON.stringify({refresh_token:session.refresh_token})});const data=await r.json();if(cancelled)return;if(!r.ok){if(r.status===400||r.status===401){sessionStorage.removeItem("legal-eye-session");setIdentity(null);toast.error("Your session expired. Sign in again.");}return;}const next={...session,access_token:data.access_token,refresh_token:data.refresh_token,expires_at:data.expires_at};sessionStorage.setItem("legal-eye-session",JSON.stringify(next));setIdentity(next);}catch{toast.error("Session refresh failed. Check your connection.");}};const delay=Math.max(1000,((session.expires_at||Date.now()/1000+300)-Date.now()/1000-60)*1000);const timer=window.setTimeout(()=>void renew(),delay);return()=>{cancelled=true;window.clearTimeout(timer)}},[identity]);
  ...
  return <div className={"legal-app view-"+view}>
    <aside className="workspace-sidebar" aria-label="LOCKE workspace navigation">
      <button className="sidebar-brand" onClick={()=>go("ask")} aria-label="LOCKE home"><LockeMark/><span><b>LOCKE</b><small>Legal intelligence.</small></span></button>
      <button className="sidebar-new" onClick={()=>go("ask")}><Plus size={18}/><span>New question</span></button>
      <div className="sidebar-scroll">
        <section className="sidebar-service-group"><small>Work</small>{primary.map(([id,label,Icon])=><button key={id} aria-current={view===id?"page":undefined} onClick={()=>go(id)}><Icon size={17}/><span>{label}</span></button>)}</section>
        <section className="sidebar-service-group"><small>Workspace</small>{context.map(([id,label,Icon])=><button key={id} aria-current={view===id?"page":undefined} onClick={()=>go(id)}><Icon size={17}/><span>{label}</span></button>)}</section>
        <section className="sidebar-service-group"><small>Intelligence</small>{intelligence.map(([id,label,Icon])=><button key={id} aria-current={view===id?"page":undefined} onClick={()=>go(id)}><Icon size={17}/><span>{label}</span></button>)}</section>
      </div>
      <div className="sidebar-integrations"><small>Integrations</small><a href="/office/word"><FilePenLine size={16}/><span>Word add-in</span><ArrowUpRight size={13}/></a><a href="/office/outlook"><MessageSquareText size={16}/><span>Outlook add-in</span><ArrowUpRight size={13}/></a></div>
      <button className="sidebar-account" onClick={()=>setAuthOpen(true)}><span className="sidebar-avatar">{identity?.user.email?.slice(0,1).toUpperCase()||"L"}</span><span><b>{identity?"Account":"Sign in"}</b><small>{identity?.user.email||"Secure workspace"}</small></span></button>
    </aside>
    <div className="workspace-stage">
      <header className="workspace-header"><button className="workspace-brand workspace-brand-mobile" onClick={()=>go("ask")} aria-label="LOCKE home"><LockeMark/><b>LOCKE</b></button><div className="workspace-head-title">{title}</div><div className="workspace-header-actions"><button aria-label="Search workspace" onClick={()=>setCommand(true)}><Search size={18}/></button><button aria-label={theme==="dark"?"Use light mode":"Use dark mode"} onClick={()=>{const t=theme==="dark"?"light":"dark";setTheme(t);document.documentElement.dataset.theme=t;localStorage.setItem("legal-eye-theme",t)}}>{theme==="dark"?<Sun size={18}/>:<Moon size={18}/>}</button><Button variant="outline" className="account-button" onClick={()=>setAuthOpen(true)}>{identity?"Account":"Sign in"}</Button><button className="mobile-menu" aria-label="Open navigation" onClick={()=>setNavOpen(!navOpen)}><Menu/></button></div></header>
      ...
      <main key={identity?`${identity.user.id}:${identity.organization_id}`:"signed-out"} className="workspace-main">{content}</main>
      <footer className="workspace-footer"><span><i/> {corpusStats.searchable===null?(identity?"Corpus status unavailable":"Sign in to view corpus coverage"):`${corpusStats.searchable.toLocaleString()} source passages`}</span><span>{corpusStats.searchable!==null?`${documentCount.toLocaleString()} legal records`:"Coverage varies by jurisdiction"}</span><span>LOCKE · Legal intelligence.</span></footer>
    </div>
    ...
    <Dialog open={command} onOpenChange={setCommand}><DialogContent className="command-dialog"><DialogHeader><DialogTitle>Go anywhere</DialogTitle><DialogDescription>Search LOCKE or open a workspace.</DialogDescription></DialogHeader><div className="command-search"><Search/><Input autoFocus placeholder="Find a workspace…" value={commandQuery} onChange={e=>setCommandQuery(e.target.value)}/></div><div className="command-list">{allNav.filter(([,label])=>label.toLowerCase().includes(commandQuery.toLowerCase())).map(([id,label,Icon])=><button key={id} onClick={()=>go(id)}><Icon/><span>{label}</span><kbd>↵</kbd></button>)}</div></DialogContent></Dialog>
    <LockeAssistant identity={identity} view={view} matterId={matterId} matters={matters} documents={vaultDocuments} connect={connect} navigate={v=>go(v as View)}/>
    <Dialog open={authOpen} onOpenChange={setAuthOpen}><DialogContent><DialogHeader><DialogTitle>{identity?"Your workspace":authMode==="signup"?"Create your LOCKE account":authMode==="verify"?"Check your email":authMode==="workspace"?"Create your firm workspace":"Sign in to LOCKE"}</DialogTitle><DialogDescription>{identity?"Your private organization workspace is active in this browser tab.":authMode==="verify"?"Confirm your email using the link we sent, then return here and sign in.":authMode==="workspace"?"Your account is verified. Give your private workspace a name to begin.":authMode==="signup"?"Register with your work email. Your firm’s documents will remain in its own private workspace.":"Use your firm account or create a new practice workspace."}</DialogDescription></DialogHeader>{authError&&<div className="inline-error" role="alert">{authError}</div>}{identity?<><div className="auth-session"><ShieldCheck/><div><b>{identity.user.email}</b><span>{identity.role} · private organization workspace</span></div><Button variant="outline" onClick={signOut}>Sign out</Button></div>{availableFirms.length>1&&<label className="auth-form">Firm workspace<select aria-label="Firm workspace" value={identity.organization_id} onChange={e=>{const firm=availableFirms.find(f=>f.organization_id===e.target.value);if(firm)void connectSession(identity,firm).catch(error=>setAuthError(error.message))}}>{availableFirms.map(f=><option key={f.organization_id} value={f.organization_id}>{f.organizations?.name||"Firm workspace"} · {f.role}</option>)}</select></label>}{pendingInvite&&<Button disabled={authBusy} onClick={async()=>{setAuthBusy(true);setAuthError("");try{await acceptInvite(identity)}catch(e){setAuthError(e instanceof Error?e.message:"Invitation failed")}finally{setAuthBusy(false)}}}>Join invited firm workspace</Button>}</>:<div className="auth-form">{pendingInvite&&<p className="helper">Sign in or register with the email your firm invited. <button type="button" className="account-recovery-back" onClick={()=>{sessionStorage.removeItem("locke-pending-invite");setPendingInvite("");setAuthError("")}}>Dismiss invitation</button></p>}{authMode==="verify"?<Button onClick={()=>{setAuthMode("signin");setAuthError("")}}>I’ve confirmed my email · sign in</Button>:authMode==="workspace"?<><label>Firm or practice name<Input value={firmName} onChange={event=>setFirmName(event.target.value)} maxLength={120} autoComplete="organization"/></label><Button onClick={()=>void createFirm()} disabled={authBusy||!firmName.trim()}>{authBusy?"Creating workspace…":"Create private workspace"}</Button><Button variant="ghost" onClick={()=>{setPendingSession(null);pendingFirmSlug.current="";setAuthMode("signin")}}>Use another account</Button></>:<><label>Email<Input type="email" value={email} onChange={event=>setEmail(event.target.value)} autoComplete="username"/></label>{authMode==="signup"&&!pendingInvite&&<label>Firm or practice name<Input value={firmName} onChange={event=>setFirmName(event.target.value)} maxLength={120} autoComplete="organization"/></label>}<label>Password<Input type="password" value={password} onChange={event=>setPassword(event.target.value)} onKeyDown={event=>event.key==="Enter"&&void (authMode==="signup"?signUp():signIn())} autoComplete={authMode==="signup"?"new-password":"current-password"} minLength={authMode==="signup"?12:undefined}/></label><Button onClick={()=>void (authMode==="signup"?signUp():signIn())} disabled={authBusy||!email||!password||(authMode==="signup"&&!pendingInvite&&!firmName.trim())}>{authBusy?"Please wait…":authMode==="signup"?"Create account":"Sign in"}</Button>{authMode==="signin"&&<a className="account-recovery-back" href="/reset-password">Forgot password?</a>}<Button variant="ghost" onClick={()=>{setAuthMode(authMode==="signup"?"signin":"signup");setAuthError("");setPassword("")}}>{authMode==="signup"?"Already have an account? Sign in":"New to LOCKE? Create an account"}</Button></>}</div>}</DialogContent></Dialog>
