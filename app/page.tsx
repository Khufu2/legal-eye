...
function Draft({show,identity,connect}:{show:()=>void;identity:DemoIdentity|null;connect:()=>void}) {
  ...
  return <div className="draft-view"><div className="doc-toolbar"><div><span className="eyebrow">Drafting workspace</span><b>New legal document</b></div><div className="row"><ToneBadge>Unsaved</ToneBadge></div></div>
    <div className="editor-grid"><aside className="outline-pane"><div className="pane-title"><span>Outline</span><Plus/></div>{["Definitions","Sale and purchase","Conditions","Pre-completion undertakings","Warranties","Limitations","Confidentiality","Governing law"].map((x,i)=><button className={i===3?"active":""} key={x}><span>{i+1}</span>{x}</button>)}</aside>
      <article className="doc-canvas"><div className="doc-page"><small>DRAFT · LAWYER REVIEW REQUIRED</small><h1>Untitled document</h1><p>Enter verified drafting context and instructions below.</p><hr/><h2>Working text</h2><Textarea value={text} onChange={e=>setText(e.target.value)} placeholder="Paste source clauses or drafting context here..."/></div></article>
...
function VaultView({identity,documents,connect,refresh}:{identity:DemoIdentity|null;documents:VaultDocument[];connect:()=>void;refresh:()=>void}) {
  ...
  const display=documents.map(d=>[d.title,d.status,d.file_name||"Private document"]);
  return <div className="view-pad"><Header title="Private knowledge" meta="Permission-filtered firm work, isolated from the public authority graph." action={<><input ref={input} hidden type="file" accept=".pdf,.docx,.xlsx,.txt" onChange={event=>upload(event.target.files?.[0])}/><Button onClick={()=>identity?input.current?.click():connect()} disabled={uploading}><Upload/> {uploading?"Securing…":"Upload"}</Button></>}/><div className="vault-grid"><section>{display.length?display.map(x=><button key={x[0]}><i><FolderLock/></i><span><b>{x[0]}</b><small>{x[1]} · {x[2]}</small></span><ChevronRight/></button>):<div className="empty-live"><FolderLock/><h2>{identity?"Your vault is empty":"Sign in to open the vault"}</h2><p>{identity?"Upload the first real client document.":"Private knowledge is available only to organization members."}</p></div>}</section><aside><ShieldCheck/><h2>Private Firm Intelligence</h2><p>Client content is permission-filtered before retrieval. Public and firm graphs stay separate and every agent action is auditable.</p>{[["Encryption","Supabase storage · private bucket"],["Data boundary","Organization + matter RLS"],["Model training","Disabled for private data"],["Parser","Queued processing"]].map(x=><div key={x[0]}><span>{x[0]}</span><b>{x[1]}</b></div>)}</aside></div></div>;
}
function TrustView() {
  const controls=[
...
function LegalEye() {
  ...
  return <SidebarProvider defaultOpen style={{"--sidebar-width":"11.75rem","--sidebar-width-icon":"3.35rem"} as React.CSSProperties}>
    ...
    <SidebarInset className="app-inset"><header className="context-head"><div><SidebarTrigger/><span><b>Legal Eye Workspace</b> <ChevronRight/> {title}</span></div><div><button className="context-pill"><Globe2/> Tanzania <ChevronDown/></button><button className="context-pill privacy-pill"><LockKeyhole/> Firm + matter</button><Button variant="outline" size="sm" onClick={()=>setCommand(true)}><CommandIcon/><span>Search</span><kbd>⌘ K</kbd></Button><button className="clock"><Clock3/></button></div></header><main>{content}</main><footer className="system-bar"><span><i/> Legal graph online</span><span>{documentCount} governed records · {count} source policies</span><span>Live data · lawyer verification required</span></footer></SidebarInset>
