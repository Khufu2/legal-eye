...
export async function POST(request:Request){
  ...
  try{
    ...
    if(input.action==="ask"){
      ...
      const generated=await generateText({model:gateway(modelName),system:"You are the client-facing Legal Eye assistant. Answer only from material explicitly published to this portal. Do not use hidden firm knowledge, unpublished workspace content, or invented authority. Every material proposition must be backed by a citation to one of the supplied resource IDs with an exact supporting quote. If the published material does not answer the question, say so clearly. This is legal work product for lawyer/client collaboration, not a substitute for final lawyer review.",prompt,output:Output.object({schema:answerSchema}),providerOptions});
