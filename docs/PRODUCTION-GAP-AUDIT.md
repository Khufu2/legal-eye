...
functional legal operating system**. The public corpus contains 20 official UK metadata records,
but zero searchable legal text chunks, structured legislation expressions, case opinions or citation
edges. The private product is not operational because there are no organizations, members, matters,
documents, playbooks, reviews, workflows, agents or monitors in the production database.
...
| Public corpus | 136 registered sources; UK and EUR-Lex policy-approved; 20 UK metadata records | Full licensed documents, structural CLML/AKN parsing, cases, courts, citations, versions, amendments, authority treatment, embeddings and reconciliation |
| African moat | 30 jurisdiction records and 12 parser plug-in registrations | Approved commercial source agreements, working connectors, Tanzanian/African full text, court hierarchy, Gazette pipeline, citation fixtures and lawyer QA |
...
| Security/compliance | RLS, security CI, audit schemas and zero current Supabase security-advisor findings | Resolve performance lints, external penetration test, threat model, DR exercise, SOC 2/ISO mapping, DPIA, vendor review and populated evidence register |
...
- 20 legal-document metadata records and 20 source-ingest objects exist.
...
- Supabase reports zero security-advisor findings. Performance advice still includes unindexed
  foreign keys and overlapping permissive RLS policies that must be resolved and benchmarked.
