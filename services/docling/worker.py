...
def structured_text_canonical(raw: str, file_name: str, content_type: str, source_hash: str, source_url: str | None = None) -> tuple[dict[str, Any], bytes]:
...
def process_job(api: SupabaseApi, job: dict[str, Any]) -> int:
    ...
    if job.get("job_type") in {"legal_xml_parse", "eurlex_structured_fetch_parse"}:
...
def main() -> None:
    ...
    parser.add_argument(
        "--job-types",
        default="official_pdf_fetch_parse,docling_parse,legal_xml_parse,eurlex_structured_fetch_parse",
