...
import {buildSearchQueries,searchTerms} from '../lib/legal/retrieval.ts';
...
test('question search retains specific legal terms and removes conversational filler',()=>assert.deepEqual(searchTerms('What makes an employment termination unfair?'),['employment','termination','unfair']));
test('long research questions build bounded legal searches instead of giant OR queries',()=>{
  const q='A Tanzanian employee who worked for a company for five years was terminated immediately for alleged misconduct without being given a disciplinary hearing. The employer paid one month salary in lieu of notice. Was the termination procedurally and substantively fair? Identify statutes, sections, leading cases and remedies.';
  assert.deepEqual(searchTerms(q).slice(0,6),['employment','termination','misconduct','disciplinary','hearing','notice']);
  const plans=buildSearchQueries(q);
  assert.equal(plans.length,3);
  assert.equal((plans.at(-1).match(/ OR /g)||[]).length,3);
  assert.doesNotMatch(plans.join(' '),/worked|company|five|years|immediately|alleged/);
});
