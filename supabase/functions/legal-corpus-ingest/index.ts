...
Deno.serve(async(request)=>{
  ...
  try{
    ...
    if(adapter!=="uk_legislation") return json({error:"Connector is not enabled"},400);
    ...
    const runs=await service("connector_runs",{method:"POST",headers:{"content-type":"application/json",Prefer:"return=representation"},body:JSON.stringify({
      source_id:source.id,connector_version:"uk-legislation@1",mode,status:"running",requested_by:user.id,
    ...
    runId=runs[0].id;
    const result=await ingestUkLegislation(source,runId,limit);
