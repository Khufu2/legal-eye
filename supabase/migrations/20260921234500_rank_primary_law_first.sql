-- Improve lawyer-facing retrieval by preferring primary legislation when a broad
-- query also matches many subsidiary instruments. Full-text relevance remains the base score.
...
eligible as materialized (
  select c.id,c.legal_document_id,c.search_vector,d.title,d.citation,d.court,
...
candidates as materialized (
  select m.id chunk_id,m.legal_document_id,m.title,m.citation,m.court,m.jurisdiction_code,
         (
           ...
           + case when to_tsvector('simple',coalesce(m.title,'')) @@ q.query then 0.22 else 0 end
