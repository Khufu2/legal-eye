...
select r.chunk_id,r.legal_document_id,r.title,r.citation,r.court,r.jurisdiction_code,c.paragraph_number,c.page_number,coalesce(context.content,c.content),r.rank,r.canonical_url,r.current_status,r.effective_from,r.effective_to
from chosen r join public.legal_document_chunks c on c.id=r.chunk_id
left join lateral (
 select string_agg('[Source PDF page ' || n.page_number || ']' || E'\n' || n.content,E'\n\n' order by n.page_number,n.reading_order nulls last,n.id) content
 from (
  select x.* from public.legal_document_chunks x
  ...
  order by x.page_number,x.reading_order nulls last,x.id limit 12
 ) n
