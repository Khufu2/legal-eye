...
matching_chunks as materialized (
  -- Separate indexed predicates: an OR across two tables forced corpus-wide scans.
  select c.id,c.legal_document_id,c.search_vector,c.reading_order from public.legal_document_chunks c cross join q
  where c.search_vector @@ q.query
  union
  select c.id,c.legal_document_id,c.search_vector,c.reading_order from public.legal_documents d cross join q
  join lateral (
    select x.id,x.legal_document_id,x.search_vector,x.reading_order from public.legal_document_chunks x
    where x.legal_document_id=d.id and coalesce(x.reading_order,0)<=1
    order by x.reading_order nulls first,x.id limit 2
  ) c on true
  where to_tsvector('simple',coalesce(d.title,'')) @@ q.query
    and (p_jurisdictions is null or d.jurisdiction_code=any(p_jurisdictions))
),
eligible as materialized (
  select c.id,c.legal_document_id,c.search_vector,c.reading_order,d.title,d.citation,d.court,
         ...
         d.document_type,(to_tsvector('simple',coalesce(d.title,'')) @@ q.query) title_match
...
candidates as materialized (
  select m.id chunk_id,m.legal_document_id,m.title,m.citation,m.court,m.jurisdiction_code,
         (
           ...
           + case when m.title_match then 1.50 else 0 end
