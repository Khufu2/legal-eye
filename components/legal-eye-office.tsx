...
export function LegalEyeOffice({host}:{host:Host}){
  ...
  useEffect(()=>{if(!session)return;void (async()=>{
    ...
    }catch(error){setMessage(error instanceof Error?error.message:"Could not load Legal Eye workspace");}
  })();},[session]);
  ...
  }catch(error){setMessage(error instanceof Error?error.message:"Legal Eye action failed");}finally{setBusy(false);}};
