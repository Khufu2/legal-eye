...
type AgentEvidence = { label?: string; title?: string; citation?: string; content?: string; canonical_url?: string; jurisdiction_code?: string; page_number?: number; court?: string };
type AgentRun = { id: string; objective: string; status: string; plan: AgentStep[]; result: { executive_summary?: string; deliverable?: string; uncertainties?: string[]; evidence?: AgentEvidence[]; selected_documents?: Array<{id:string;title:string}> } | null; created_at: string };
...
export function SkillsLive({ identity, connect, documents }: CommonProps & { documents: LiveVaultDocument[] }) {
  ...
  return <div className="view-pad"><LiveHeader title="Skills" meta="Save firm instructions and run them again." action={<Button onClick={() => { setSelected(null); setName("New legal skill"); setInstructions(""); }} disabled={!identity}><Plus/> New skill</Button>}/>
    <IntelligenceGuide title="How Skills work" steps={[
      {title:"Choose or create a skill",detail:"A skill is a reusable set of firm instructions."},
      {title:"Add a Ready document when needed",detail:"Upload in Vault first; document context is optional."},
      {title:"Run the skill",detail:"LOCKE applies the saved instructions to the objective and selected file."},
      {title:"Review the result",detail:"Treat the output as lawyer work product that still needs verification."},
    ]}/>
    ...
    {!identity ? <SecureEmpty identity={identity} connect={connect} title="No skills yet" copy="Create your first reusable skill."/> : <div className="skills-grid"><div className="skill-list">{skills.map(s => <button className={selected === s.id ? "active" : ""} key={s.id} onClick={() => setSelected(s.id)}><i><Sparkles/></i><span><b>{s.name}</b><small>{s.description || "Firm legal skill"}</small></span><ChevronRight/></button>)}</div><section className="skill-detail"><div className="skill-head"><div><span className="eyebrow">Firm skill</span><Input value={name} onChange={e => setName(e.target.value)}/><p>Instructions, permissions and lawyer checkpoints stay inside your organization.</p></div><div className="row"><Button variant="outline" onClick={save} disabled={busy || !name.trim() || !instructions.trim()}>Save</Button><Button onClick={run} disabled={busy || !instructions.trim()}><Play/> Run</Button></div></div><div className="instructions"><label className="skill-document-picker"><span>Private document <em>optional</em></span><select value={selectedDocument} onChange={e=>setSelectedDocument(e.target.value)}><option value="">No private document</option>{readyDocuments.map(doc=><option value={doc.id} key={doc.id}>{doc.title}</option>)}</select></label><label className="intelligence-field"><span>Skill instructions</span><small>Reusable firm rules LOCKE should follow every time this skill runs.</small><Textarea value={instructions} onChange={e => setInstructions(e.target.value)} placeholder="Define the firm's rules, fallback positions and output requirements…"/></label><label className="intelligence-field"><span>Run objective</span><small>What should this specific run produce?</small><Input value={objective} onChange={e => setObjective(e.target.value)} placeholder="e.g. Review this NDA and produce a risk memo"/></label>{result ? <div className="agent-result"><span className="eyebrow">Latest run</span><pre>{result}</pre></div> : null}</div></section></div>}
  </div>;
}
...
export function AgentLive({ identity, connect, documents }: CommonProps & { documents: LiveVaultDocument[] }) {
  ...
  const run = async () => { if (!identity) return connect(); setBusy(true); try {
    const data=await legalWork<{run:AgentRun}>(identity,{action:"run_agent",objective,document_ids:picked,persist:true});
    setCurrent(data.run);toast.success("Agent output saved for lawyer review",{description:"The timeline records tools that actually ran."});await load();
  } catch(error){toast.error(error instanceof Error?error.message:"Agent run failed");await load();}finally{setBusy(false);} };
  const deleteRun = async (id:string) => {
    if(!identity) return;
    if(!window.confirm("Delete this saved agent run? This removes the stored deliverable from your workspace.")) return;
    try {
      await rest(identity, `agent_runs?id=eq.${id}&organization_id=eq.${identity.organization_id}`, { method:"DELETE", headers:{ Prefer:"return=minimal" } });
      if(current?.id===id) setCurrent(null);
      toast.success("Agent run deleted");
      await load();
    } catch(error) { toast.error(error instanceof Error ? error.message : "Agent run could not be deleted"); }
  };
  const progress = current?.plan?.length ? current.plan.filter(s => s.status === "complete").length / current.plan.length * 100 : 0;
  return <div className="view-pad"><LiveHeader title="Agent" meta="Give LOCKE an objective. Add private files only when needed." action={<Button onClick={run} disabled={busy || !objective.trim()}><Play/> {busy ? "Running…" : "Run agent"}</Button>}/>
    <IntelligenceGuide title="How Agent works" steps={[
      {title:"State the legal objective",detail:"Describe the memo, analysis or work product you need."},
      {title:"Select only relevant private files",detail:"Agent can work without private files, or read only the Ready files you tick."},
      {title:"Run the agent",detail:"It can search approved primary-law sources and read selected firm documents."},
      {title:"Review and export",detail:"Check sources and uncertainties before downloading the lawyer work product."},
    ]}/>
    {!identity ? <SecureEmpty identity={identity} connect={connect} title="No agent runs" copy="Sign in to run governed legal work."/> : <><div className="agent-input intelligence-input-card"><div className="workbench-label"><i>1</i><span><b>Define the work product</b><small>State the memo, analysis, comparison or drafting task you want completed.</small></span></div><Textarea aria-label="Agent objective" value={objective} onChange={e => setObjective(e.target.value)} /><div className="workbench-label"><i>2</i><span><b>Add private context</b><small>Optional. Agent can only read Ready files you explicitly select.</small></span></div><div className="live-source-picker"><span className="eyebrow">{picked.length ? `${picked.length} file${picked.length===1?"":"s"} selected` : "No private files selected"}</span>{ready.length?ready.map(doc => <label key={doc.id}><Checkbox checked={picked.includes(doc.id)} onCheckedChange={checked => setPicked(prev => checked ? [...new Set([...prev, doc.id])] : prev.filter(id => id !== doc.id))}/><span>{doc.title}</span></label>):<a className="inline-vault-link" href="/?view=vault">Upload a document in Vault</a>}</div></div><div className="workbench-label result-label"><i>3</i><span><b>Review the result</b><small>Inspect the work product, primary-law evidence and uncertainties before export.</small></span></div><div className="agent-grid"><section className="agent-card">{current ? <><div className="run-head"><i><Bot/></i><div><span className="eyebrow">Persisted agent run</span><h2>{current.objective}</h2></div><Status value={current.status}/></div><div className="progress-block"><div className="row spread"><span>Execution</span><b>{Math.round(progress)}%</b></div><Progress value={progress}/></div><div className="timeline">{(current.plan || []).map(step => <div key={step.step} className={step.status === "complete" ? "done" : step.status === "needs_review" ? "active" : ""}><i>{step.status === "complete" ? <Check/> : step.step}</i><span><b>{step.title}</b><small>{step.detail}</small></span><Status value={step.status}/></div>)}</div></> : <SecureEmpty identity={identity} connect={connect} title="No agent run yet" copy="Define an objective and execute it."/>}</section><aside className="agent-side">{current?.result ? <><span className="eyebrow">Deliverable</span><Button variant="outline" onClick={()=>downloadDocx("LOCKE agent work product",`${current.result?.executive_summary||""}\n\n${current.result?.deliverable||""}`)}><Download/> Word</Button><h2>{current.result.executive_summary}</h2>{current.result.selected_documents?.length?<div className="agent-used-documents"><span className="eyebrow">Private documents used</span>{current.result.selected_documents.map(doc=><span key={doc.id}><FileText/>{doc.title}</span>)}</div>:null}<pre>{current.result.deliverable}</pre>{current.result.evidence?.length?<div className="agent-evidence"><span className="eyebrow">Primary-law evidence used</span>{current.result.evidence.map((source,index)=><article key={(source.label||"P")+index}><b>{source.label||`P${index+1}`} · {source.title||"Legal authority"}</b><small>{source.citation||source.jurisdiction_code||"Primary law"}{source.page_number?` · p. ${source.page_number}`:""}</small>{source.content?<blockquote>{source.content}</blockquote>:null}{source.canonical_url&&/^https?:\/\//.test(source.canonical_url)?<a href={source.canonical_url} target="_blank" rel="noopener noreferrer">Open source</a>:null}</article>)}</div>:null}{current.result.uncertainties?.length ? <div className="uncertainties"><b>Uncertainties</b>{current.result.uncertainties.map(x => <p key={x}>{x}</p>)}</div> : null}</> : <p>Agent output will appear here.</p>}<hr/><span className="eyebrow">Recent runs</span>{runs.map(runItem => <div className="agent-run-row" key={runItem.id}><button onClick={() => setCurrent(runItem)}><span>{runItem.objective}</span><Status value={runItem.status}/></button><button className="icon-danger" aria-label="Delete agent run" title="Delete saved run" onClick={()=>void deleteRun(runItem.id)}><Trash2/></button></div>)}</aside></div></>}
  </div>;
}
...
export function MonitorsLive({ identity, connect, documents, stats, total }: CommonProps & { documents: LiveCorpusDocument[]; stats: LiveCorpusStats; total: number }) {
  ...
  return <div className="view-pad"><LiveHeader title="Monitors" meta="Track governed source changes and turn them into reviewable follow-up." action={<Button variant="outline" onClick={() => void load()} disabled={!identity}><RefreshCw/> Refresh</Button>}/>
    <IntelligenceGuide title="How Monitors work" steps={[
      {title:"Name the issue to watch",detail:"Create a topic monitor and choose the legal area you care about."},
      {title:"Scan governed sources",detail:"LOCKE checks approved catalogue changes for relevant updates."},
      {title:"Triage the event",detail:"Open the official source and decide whether it matters to the firm."},
      {title:"Assign or resolve",detail:"Keep follow-up visible until the legal impact has been dealt with."},
    ]}/>
    <div className="corpus-strip"><div><i><Globe2/></i><span><small>Tanzania corpus</small><b>{stats.TZ.toLocaleString()}</b><em>official records</em></span></div><div><i><Globe2/></i><span><small>UK + EU</small><b>{(stats.UK + stats.EU).toLocaleString()}</b><em>governed records</em></span></div><div><i><Search/></i><span><small>Searchable chunks</small><b>{(stats.searchable || 0).toLocaleString()}</b><em>exact passages</em></span></div><div><i><Activity/></i><span><small>Total catalogue</small><b>{total.toLocaleString()}</b><em>records</em></span></div></div>{!identity ? <SecureEmpty identity={identity} connect={connect} title="No monitors yet" copy="Sign in to create a monitor."/> : <><div className="monitor-create-card"><div className="workbench-label"><i>1</i><span><b>Create a legal watch</b><small>Name the issue and describe the source change you want LOCKE to surface.</small></span></div><div className="live-create-bar"><label className="intelligence-field"><span>Monitor name</span><Input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Tanzania employment regulation changes"/></label><label className="intelligence-field"><span>Topic to watch</span><Input value={topic} onChange={e => setTopic(e.target.value)} placeholder="e.g. termination, disciplinary procedure, labour rules"/></label><Button onClick={create} disabled={busy || !name.trim()}><Plus/> Create monitor</Button></div></div><div className="monitor-live-grid"><aside>{monitors.map(m => <article key={m.id}><Activity/><div><b>{m.name}</b><small>{m.topic_query || m.monitor_type} · {(m.jurisdiction_codes || []).join(", ")}</small></div><Status value={m.status}/><Button size="sm" variant="ghost" onClick={()=>void toggleMonitor(m)}>{m.status==='active'?'Pause':'Resume'}</Button><Button size="sm" variant="outline" onClick={() => void scan(m)} disabled={busy||m.status!=="active"}>Scan</Button></article>)}</aside><section>{events.length ? events.map(event => <article key={event.id}><div><time>{new Date(event.detected_at).toLocaleDateString()}</time><Status value={event.status}/></div><h2>{event.title}</h2><p>{event.summary}</p><footer><Button variant="outline" disabled={!event.source_url} onClick={() => event.source_url && window.open(event.source_url, "_blank", "noopener,noreferrer")}>Official source</Button><Button variant="ghost" onClick={() => void triage(event, "triaged")}>Triage</Button><Button variant="ghost" onClick={() => void triage(event, "assigned")}>Assign to me</Button><Button variant="ghost" onClick={() => void triage(event, "resolved")}>Resolve</Button><Button variant="ghost" onClick={() => void triage(event, "dismissed")}>Dismiss</Button></footer></article>) : <SecureEmpty identity={identity} connect={connect} title="No monitor events yet" copy="Create a monitor and scan the governed corpus."/>}</section></div></>}
  </div>;
}
