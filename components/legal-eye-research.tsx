...
type Turn = { question: string; answer: string; evidence: Evidence[]; created_at: string };
...
export function ResearchLive({ identity, connect, initialQuery, autoRun, sessionId, onSession, jurisdictions, setJurisdictions, matterId, privateContext }: Props) {
  ...
  const run = async (value = question) => {
    ...
    try {
      const result = await action<{ answer: string; publicEvidence: Evidence[]; privateEvidence: Evidence[] }>(identity, "/api/legal-ai", { action: "research", query: value.trim(), jurisdictions, matter_id: scope.matterId, use_firm_knowledge: scope.privateContext, conversation: turns.slice(-4).map(t => ({ question: t.question, answer: t.answer.slice(0, 6000) })) });
      ...
      const next = [...turns, { question: value.trim(), answer: result.answer, evidence, created_at: new Date().toISOString() }];
...
export function ResearchHome({ identity, go, navigate, openSession, jurisdictions, setJurisdictions, privateContext, setPrivateContext }: HomeProps) {
  ...
  return <div className="cathedral-home"><section className="home-hero"><h1>Ask what matters.</h1><p>Research the law, examine the evidence, and turn<br className="desktop-break"/> your understanding into better legal work.</p><form className="home-composer" onSubmit={e => { e.preventDefault(); go(question); }}><label htmlFor="legal-question">Ask a legal question</label><textarea id="legal-question" placeholder="What do you want to understand?" value={question} onChange={e => setQuestion(e.target.value)} onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); go(question); } }}/><div className="composer-controls"><button type="button" aria-label="Open private files" onClick={() => navigate("vault")}><Plus/></button><button type="button" onClick={() => setFilters(!filters)}><BookOpen size={16}/> {jurisdictions.length === 1 ? JURISDICTIONS.find(j => j[0] === jurisdictions[0])?.[1] : `${jurisdictions.length} jurisdictions`}<ChevronDown size={13}/></button><button type="button" className={privateContext ? "context-active" : ""} onClick={() => setPrivateContext(!privateContext)}><ShieldCheck size={15}/>{privateContext ? "Firm + law" : "Public law"}</button><span className="composer-spacer"/><button type="button" onClick={() => setFilters(!filters)} aria-expanded={filters}><SlidersHorizontal size={16}/><span className="filter-label">Filter</span></button><Button type="submit" aria-label="Ask LOCKE" disabled={question.trim().length < 3}><ArrowUp/></Button></div>{filters && <div className="jurisdiction-picker"><span>Research jurisdictions</span>{JURISDICTIONS.map(([code, name]) => <label key={code}><input type="checkbox" checked={jurisdictions.includes(code)} onChange={e => setJurisdictions(e.target.checked ? [...jurisdictions, code] : jurisdictions.filter(j => j !== code).length ? jurisdictions.filter(j => j !== code) : [code])}/>{name}</label>)}<p>Coverage varies by source. Answers identify gaps in the retrieved evidence.</p></div>}</form><div className="question-examples">{["What makes an employment termination unfair?", "Compare directors’ duties in Tanzania and the UK.", "What should I look for in a confidentiality agreement?"].map(q => <button key={q} onClick={() => setQuestion(q)}>{q}</button>)}</div></section>
    <section className="home-work"><div className="home-section-heading"><div><h2>Your legal workspace</h2><p>Bring the question, the documents, and the next step together.</p></div><button onClick={() => navigate("matters")}>Open matters <ArrowUpRight size={15}/></button></div><div className="work-shortcuts">{[["review", "Review an agreement", "Find risks and propose better wording.", FileText], ["draft", "Prepare a document", "Draft, refine, and keep every version.", BookOpen], ["tables", "Compare documents", "Extract answers with source evidence.", Search], ["workflows", "Build a workflow", "Connect steps and lawyer approvals.", Check]].map(([view, title, copy, Icon]) => { const I = Icon as typeof Search; return <button key={String(view)} onClick={() => navigate(String(view))}><I size={20}/><h3>{String(title)}</h3><p>{String(copy)}</p><ArrowUpRight size={16}/></button>; })}</div><div className="home-section-heading recent-heading"><h2>Continue working</h2><span>{recent.length ? "Recent research" : "Your work, in one place"}</span></div>{recent.length ? <div className="recent-research">{recent.map(r => <button key={r.id} onClick={() => openSession(r.id)}><MessageSquareText size={18}/><b>{r.title}</b><time>{new Date(r.updated_at).toLocaleDateString()}</time><ArrowUpRight size={16}/></button>)}</div> : <p className="home-empty">{identity ? "Start your first question. Research and its evidence are saved here automatically." : "Sign in to save research, organise matters, and work with private documents."}</p>}</section>
    <section className="home-guide" aria-label="How to use LOCKE">
